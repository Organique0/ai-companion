...
export class MemoryManager {
    ...
    public async vectorSearch(recentChatHistory: string, companionFileName: string) {
        ...
        const vectorStore = await PineconeStore.fromExistingIndex(
            new OpenAIEmbeddings({ openAIApiKey: process.env.OPEN_API_KEY! }), { pineconeIndex }
        );
    ...
    public async addToHistory(companionKey: CompanionKey, text: string) {
        if (!companionKey || typeof companionKey.userId == "undefined") {
            console.log("Companion key is not defined");
            return "";
        }
